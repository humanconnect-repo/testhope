-- Sicurezza avanzata per il campo is_admin
...
-- 3. Crea una policy RLS più restrittiva per is_admin
...
CREATE POLICY "Users can update their own profile" ON profiles
  ...
  WITH CHECK (
    wallet_address = auth.jwt() ->> 'sub'
    AND is_admin = OLD.is_admin  -- Impedisce la modifica di is_admin
    AND wallet_address = OLD.wallet_address  -- Impedisce la modifica di wallet_address
  );
