-- Funzioni semplici e funzionanti
...
-- 2. Funzione semplice per ottenere le prediction con percentuali
...
BEGIN
  SELECT json_agg(
    json_build_object(
      ...
      'yes_percentage', COALESCE(
        (SELECT ROUND(
          (COUNT(*) FILTER (WHERE b.position = 'yes')::NUMERIC / NULLIF(COUNT(*), 0)::NUMERIC) * 100, 
          2
        ) FROM bets b WHERE b.prediction_id = p.id), 
        0
      ),
      'no_percentage', COALESCE(
        (SELECT ROUND(
          (COUNT(*) FILTER (WHERE b.position = 'no')::NUMERIC / NULLIF(COUNT(*), 0)::NUMERIC) * 100, 
          2
        ) FROM bets b WHERE b.prediction_id = p.id), 
        0
      ),
      'total_bets', COALESCE(
        (SELECT COUNT(*) FROM bets b WHERE b.prediction_id = p.id), 
        0
      ),
      'total_amount_bnb', COALESCE(
        (SELECT SUM(b.amount_bnb) FROM bets b WHERE b.prediction_id = p.id), 
        0
      )
    )
  ) INTO result
...
-- 3. Funzione semplice per ottenere le percentuali di una singola prediction
...
-- 4. Funzione semplice per ottenere i top scommettitori
...
-- 5. Funzione semplice per ottenere i commenti
...
SELECT 'Funzioni semplici create con successo!' as status;
