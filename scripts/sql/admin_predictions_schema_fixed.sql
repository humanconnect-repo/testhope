...
-- VERSIONE CORRETTA - Compatibile con la struttura esistente

-- 1. Aggiungi colonna is_admin alla tabella profiles esistente
ALTER TABLE profiles 
ADD COLUMN IF NOT EXISTS is_admin BOOLEAN DEFAULT FALSE;

-- 2. Crea tabella predictions (SENZA foreign key per created_by per ora)
CREATE TABLE IF NOT EXISTS predictions (
  ...
  created_by TEXT, -- Cambiato da UUID a TEXT per compatibilità
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 3. Crea tabella bets per le scommesse (SENZA foreign key per user_id per ora)
CREATE TABLE IF NOT EXISTS bets (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  prediction_id UUID REFERENCES predictions(id) ON DELETE CASCADE,
  user_id TEXT, -- Cambiato da UUID a TEXT per compatibilità
  amount_bnb DECIMAL(18,8) NOT NULL,
  position TEXT NOT NULL CHECK (position IN ('yes', 'no')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 4. Crea tabella comments per i commenti (SENZA foreign key per user_id per ora)
...
-- 11. Inserisci alcune prediction di esempio
